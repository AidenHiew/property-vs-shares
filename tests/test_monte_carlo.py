...
def test_wealth_paths_have_correct_shape():
    """Year-by-year wealth arrays should be shape (trials, horizon_years)."""
    result = run_monte_carlo(
        trials=200, horizon_years=10,
        purchase_price=700_000, deposit=140_000,
        stamp_duty=30_000, buying_costs=2_600,
    ...
    assert result["property_wealth_path"].shape == (200, 10)
    assert result["shares_wealth_path"].shape == (200, 10)
    assert result["mixed_wealth_path"].shape == (200, 10)


def test_wealth_path_is_monotonic_average():
    """Wealth path should be increasing on average — both assets appreciate."""
    import numpy as np
    result = run_monte_carlo(
    ...
    # Median wealth at year 10 > median wealth at year 1, for both strategies
    median_path_prop = np.median(result["property_wealth_path"], axis=0)
    median_path_shr = np.median(result["shares_wealth_path"], axis=0)
    assert median_path_prop[-1] > median_path_prop[0]
    assert median_path_shr[-1] > median_path_shr[0]


def test_mixed_wealth_path_is_weighted_average():
    """mixed_wealth_path[t,y] = mix * property[t,y] + (1-mix) * shares[t,y]."""
    import numpy as np
    result = run_monte_carlo(
        trials=200, horizon_years=10,
        purchase_price=700_000, deposit=140_000,
    ...
    expected = 0.6 * result["property_wealth_path"] + 0.4 * result["shares_wealth_path"]
    np.testing.assert_array_almost_equal(result["mixed_wealth_path"], expected)
